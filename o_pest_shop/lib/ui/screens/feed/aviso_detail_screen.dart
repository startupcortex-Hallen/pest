...
class AvisoDetailScreen extends StatefulWidget {
  final String postId;
  const AvisoDetailScreen({super.key, required this.postId});

  @override
  State<AvisoDetailScreen> createState() => _AvisoDetailScreenState();
}

class _AvisoDetailScreenState extends State<AvisoDetailScreen> {
  ...
  Future<void> _loadData() async {
    try {
      ...
      if (postJson != null) _post = FeedPost.fromJson(postJson as Map<String, dynamic>);

      _comments = await _feedService.fetchComments(widget.postId);
    } catch (e) {
      debugPrint('Erro aviso detail: $e');
    }
  ...
  Widget _buildMediaSection() {
    ...
    if (img != null && img.isNotEmpty) {
      return ClipRRect(
        ...
        child: Hero(
          tag: 'post_${p.id}',
          child: CachedNetworkImage(imageUrl: img, height: 200, width: double.infinity, fit: BoxFit.cover,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _loading
          ...
          : _post == null
              ? const Center(child: Text('Aviso não encontrado'))
              : Column(
                  children: [
                    Expanded(
                      child: ListView(
